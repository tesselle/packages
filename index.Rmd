...
```{r check}
check <- colSums(status_table[, -c(1, 2)])
worst <- max(which(check > 0))
cap <- c("OK", "Note", "Warning", "Error", "Fail")
col <- c("success", "info", "warning", "danger", "danger")
flexdashboard::valueBox(cap[worst], icon = "fab fa-r-project", 
                        href = "#cran-check-status", color = col[worst])
```
...
issues <- vapply(
  X = info$Package,
  FUN = function(x) {
    api <- sprintf("https://api.github.com/repos/tesselle/%s/issues", x)
    doc <- try(rjson::fromJSON(file = api), silent = TRUE)
    if (inherits(doc, "try-error")) return(0)
    length(doc)
  },
  FUN.VALUE = numeric(1)
)
flexdashboard::valueBox(sum(issues), icon = "fab fa-github", href = NULL, 
                        color = ifelse(sum(issues) > 0, "warning", "primary"))
```
...
pulls <- vapply(
  X = info$Package,
  FUN = function(x) {
    api <- sprintf("https://api.github.com/repos/tesselle/%s/pulls", x)
    doc <- try(rjson::fromJSON(file = api), silent = TRUE)
    if (inherits(doc, "try-error")) return(0)
    length(doc)
  },
...
flexdashboard::valueBox(sum(pulls), icon = "fab fa-github", href = NULL, 
                        color = ifelse(sum(pulls) > 0, "warning", "primary"))
```
